...
<div class="row row-cols-3 row-cols-lg-5 g-2 g-lg-4 h-100">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['strCategory'])) {
        ...
        foreach ($meals as $meal) {
            ...
            echo "<img id='meal-image-{$meal['meal_name']}-{$meal['id']}' src='{$meal['image_path']}' class='img-fluid mx-auto d-block w-100' alt='{$meal['meal_name']}' onerror=\"this.src='../../assets/images/placeholder.jpg'\">";
            echo "</div>";
            echo "</div>";
            echo "</div>";
